...
class InputLatencyTests(unittest.TestCase):
    def line(self, raw="1:10,2:20,3:30,4:40"):
        return "input-latency: session_id=7 authorization_epoch=9 samples=4 min_us=10 p50_us=20 p95_us=40 p99_us=40 max_us=40 mean_us=25 raw_us=" + raw
    ...
    def test_tampered_summary(self):
        parsed = probe.parse_input_latency(self.line().replace("mean_us=25", "mean_us=99"))
        self.assertTrue(any("summary" in e for e in probe.validate_probe(parsed, 4, [(7, 9)], [(7, 9)])))

    def test_gaps_and_duplicates(self):
        parsed = probe.parse_input_latency(self.line("1:10,2:20,2:30,4:40"))
        self.assertTrue(any("contiguous" in e for e in probe.validate_probe(parsed, 4, [(7, 9)], [(7, 9)])))

    def test_wrong_identity_and_epoch(self):
        parsed = probe.parse_input_latency(self.line())
        self.assertTrue(any("lifecycle" in e for e in probe.validate_probe(parsed, 4, [(8, 9)], [(7, 10)])))

    def test_p95_ceiling(self):
        parsed = probe.parse_input_latency(self.line("1:10,2:20,3:30,4:100001"))
        self.assertTrue(any("ceiling" in e for e in probe.validate_probe(parsed, 4, [(7, 9)], [(7, 9)])))
