...
| Verified — concurrent Linux multi-target input application-ACK latency (Xvfb/headless) | One supervisor launches two exact-pinned children against two distinct-certificate Hosts. Flushed full-stamp start/stop markers prove both 256-sample probe intervals overlap; every target independently passes raw sequence/statistic, route, lifecycle, certificate, mTLS, real-stream, ReleaseAll, exit, cleanup, and binary-hash checks | `xvfb-run -a python3 scripts/multi_target_input_latency_test.py --host-bin target/debug/latencydesk-host --client-bin target/debug/latencydesk-client --identity-bin target/debug/latencydesk-identity --samples 256 --timeout 45 --output artifacts/multi-target-input-latency.json`; two-Host single-machine application ACK RTT only, excluding 4/8/16-Host scale, physical input-to-photon, cross-machine/network-shaped workloads, resource soak, and AnyDesk/RustDesk comparison |
