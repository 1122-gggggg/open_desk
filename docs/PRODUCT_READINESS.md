...
| Partial | Reconnect and network handoff | Linux X11 Host and headless Client execute a bounded automatic clean successor sequence with fresh lifecycle stamps and ReleaseAll between sessions; abrupt-loss and interactive recovery, Windows Host persistence, cable/Wi-Fi handoff, suspend/resume, and the declared recovery-time target remain Pending |
